// ══════════════════════════════════════════════════════════════
//  PONTO DIGITAL — app.js v11.1 (God Mode Telemetry Integrated)
  ...
  if (s) {
    try {
      ...
      if (sessao && sessao.nome) {
        // iniciarApp() vai disparar o GodModeTracker.loginSuccess
  ...
  try {
    ...
    .then(function (d) {
      if (d.status === 'ok') {
        ...
        esconderLogin();
        iniciarApp(); // dispara loginSuccess do tracker dentro
      } else {
        err.textContent = d.msg || 'Credenciais inválidas';
        shakeLogin();

        // ── Telemetria: tentativa de login falhou ──
        if (window.GodModeTracker) {
          GodModeTracker.loginFailure({
            usuario: user,
            motivo: d.msg || 'Credenciais inválidas'
          });
        }
      }
    })
    .catch(function () {
      if (CREDS_OFFLINE[user] && CREDS_OFFLINE[user] === senhaHash) {
        ...
        // ── Telemetria: falha de login offline ──
        if (window.GodModeTracker) {
          GodModeTracker.loginFailure({
            usuario: user,
            motivo: 'Sem conexão e credenciais inválidas'
          });
        }
      }
    })
    ...
    err.textContent = 'Erro de segurança';
    shakeLogin();
...
function logout() {
  // ── Telemetria: logout ANTES de zerar a sessão ──
  if (window.GodModeTracker) {
    GodModeTracker.logout();
  }
  ...
  dadosGestorCache = null;
  document.getElementById('viagemSwitchToggle').classList.remove('on');
  toggleViagemLogic(false);
  document.getElementById('areaOperacional').style.display = 'block';
}

document.addEventListener('DOMContentLoaded', function () {
  // ── Inicializa telemetria God Mode (apenas init; loginSuccess é em iniciarApp) ──
  if (window.GodModeTracker) {
    GodModeTracker.init({
      idCliente: 'crv',
...
function iniciarApp() {
  // ── Telemetria: notifica sessão ativa (login novo OU sessão restaurada) ──
  if (window.GodModeTracker && sessao && sessao.nome) {
    GodModeTracker.loginSuccess({
      usuario: sessao.nome,
  ...
  loadSequence([
    { t: 'Autenticando...', p: 20 },
    { t: 'Preparando módulo...', p: 50 },
    { t: 'Calibrando GPS...', p: 80 },
    { t: 'Pronto!', p: 100 }
  ], function () {
...
function capturePhoto() {
  ...
  selfieData = c.toDataURL('image/jpeg', 0.4);
...
function initGeo() {
  ...
  navigator.geolocation.watchPosition(function (pos) {
    ...
    updateGeoUI('GPS negado', 'red', 'Ative o GPS para registrar.');
...
function registrarPonto() {
  ...
  var payload = {
    nome: sessao.nome, tipo: tipoSelecionado, selfie: selfieData,
    lat: geoAtual.lat, lng: geoAtual.lng,
    dispositivo: navigator.userAgent.substring(0, 60), ip: '',
    viagem: isViagem, destino: destinoText
  };
  ...
  .catch(function (err) {
    toast('Registro local. Sincronizando...');
    // ── Telemetria: erro de registro ──
    if (window.GodModeTracker) {
      GodModeTracker.log({
        tipo: 'ERRO',
        mensagem: 'Falha ao registrar ponto: ' + (err && err.message ? err.message : 'rede')
      });
    }
  });
}
...
function showSuccess(d) {
  ...
  document.getElementById('successMsg').textContent = d.mensagem || 'Registro Salvo!';
...
function syncDados() {
  fetch(API_URL + '?sync=1')
    .then(function (r) { return r.json(); })
    .then(function (d) { setBadge(true); if (d.timeline) renderTimeline(d.timeline, 'timelineList'); })
...
function abrirLista(tipo) {
  ...
  if (tipo === 'viagem') {
    ...
    var passouDaHora = minutosHoje > 490;
  ...
  fetch(API_URL + '?painel=1&senha=' + encodeURIComponent(sessao.senha))
    ...
    .then(function (d) {
      ...
      nomes.forEach(function (n) {
        if (d.colaboradores[n].registros.some(r => r.statusGeo.indexOf('VIAGEM') > -1)) { emViagem.push(n); }
      });
      document.getElementById('statViagem').textContent = emViagem.length;

      var todasAtividades = [];
      nomes.forEach(n => {
        d.colaboradores[n].registros.forEach(r => {
          todasAtividades.push({ nome: n, hora: r.hora, tipo: r.tipo });
        });
      });
  ...
  nomes.forEach(function (n) {
    ...
    c.registros.forEach(function (r) {
      if (r.statusGeo.indexOf('VIAGEM') > -1) {
        var match = r.obs.match(/DESTINO:\s([^|]+)/);
        var dest = match ? match[1].trim() : 'Rota Indefinida';
        if (!rotas[dest]) rotas[dest] = [];
        if (rotas[dest].indexOf(n) === -1) rotas[dest].push(n);
      }
    });
    if (c.saida) st = 'Saiu às ' + c.saida;
    else if (c.retornoAlmoco) st = 'Retornou ' + c.retornoAlmoco;
    ...
    if (!pres && !c.justificativa) {
      h += '<button onclick="abrirJustificativa(\'' + n + '\')" class="cam-btn" style="background:var(--orange-soft); color:var(--orange);">Justificar</button>';
    } else {
      h += '<div style="font-weight:700; font-family:var(--font-rounded);">' + (c.entrada || '—') + '</div>';
    }
    h += '</div>';
  });
...
function abrirJustificativa(nome) {
  document.getElementById('justNome').textContent = nome;
  document.getElementById('justNomeInput').value = nome;
...
function salvarJustificativa() {
  ...
  var nomeColab = document.getElementById('justNomeInput').value;
  if (motivo.length < 3) { toast('Escreva um motivo válido'); return; }
  var btn = document.getElementById('btnSalvarJust');
  btn.disabled = true; btn.textContent = 'Salvando...';
  fetch(API_URL, {
    method: 'POST',
    headers: { 'Content-Type': 'text/plain;charset=utf-8' },
    body: JSON.stringify({ acao: 'justificar', nomeColab: nomeColab, motivo: motivo, gestor: sessao.nome }),
    redirect: 'follow'
  })
    .then(function (r) { return r.json(); })
    .then(function (d) {
      if (d.status === 'ok') { fecharJustificativa(); toast(d.msg); carregarPainel(); }
      else { toast('Erro ao justificar'); }
    })
    .catch(function () { toast('Sem conexão'); })
    .finally(function () { btn.disabled = false; btn.textContent = 'Salvar Justificativa'; });
}
